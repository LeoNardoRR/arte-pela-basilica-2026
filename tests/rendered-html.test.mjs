...
test("catalog provides flexible prices, full-screen gallery and timed pre-reservation flow", async () => {
  ...
  assert.match(emailWorker, /reservation_email_queue/);
  assert.match(emailWorker, /function loadEnvFile/);
  assert.match(emailWorker, /BREVO_API_KEY/);
  assert.match(emailWorker, /SUPABASE_ACCESS_TOKEN/);
  assert.match(emailWorker, /database\/query/);
...
test("SOC hardening denies public PII RPCs and removes the duplicate D1 surface", async () => {
  ...
  const communicationMigration = await read("supabase/migrations/20260916120000_reservation_email_queue.sql");
  assert.match(communicationMigration, /2026-09-23 00:00:00-03/);
  assert.doesNotMatch(communicationMigration, /perform public\.release_expired_pre_reservations\(\);/);
