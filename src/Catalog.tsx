...
const statusLabel = { available: "Disponível", sold: "Adquirida" } as Record<string, string>;

const money = new Intl.NumberFormat("pt-BR", { style: "currency", currency: "BRL" });

export function Catalog({ onAdmin }: { onAdmin?: () => void }) {
  ...
  const scrollCarousel = (direction: 'left' | 'right') => {
    if (carouselRef.current) {
      const scrollAmount = window.innerWidth > 950 ? 400 : 300;
  ...
  async function submitBidCart(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch(`${SUPABASE_URL}/rest/v1/rpc/submit_auction_cart`, {
        ...
        body: JSON.stringify({ bidder_name: form.get("name"), bidder_email: form.get("email"), bidder_phone: form.get("phone"), preferred_payment_method: "local", items: cart.map((item) => ({ artwork_id: item.work.id, amount_cents: Math.round(item.amount * 100) })) }),
      });
      ...
      setCart([]);
      setMessage("Proposta enviada. A equipe confirmará os lances vencedores no evento.");
  ...
  return <main>
    <header className="site-header">
      ...
      <nav aria-label="Navegação principal"><a href="#evento">O evento</a><a href="#acervo">Obras de arte</a><a href="#como-comprar">Como participar</a><a href="#contato">Fale conosco</a>{onAdmin && <a href="#admin" onClick={(e) => { e.preventDefault(); onAdmin(); }} style={{ color: "var(--gold)" }}>Administrador</a>}</nav>
      <button className={`cart-trigger ${cartBouncing ? 'bouncing' : ''}`} onClick={() => setCartOpen(true)} aria-label={`Abrir sacola com ${cart.length} obras`}><span>Minha sacola</span><b>{cart.length}</b></button>
    </header>
    ...
    <aside className="event-bar" id="evento"><div className="event-icon" aria-hidden="true">◇</div><div><strong>10 de setembro de 2026</strong></div><div><span>Local</span><strong>Hotel anfitrião · detalhes em breve</strong></div><div><span>Acervo online</span><strong>Disponível até 17 de setembro</strong></div><a href="#acervo">Explorar <span>→</span></a></aside>
    ...
    <section className="catalog-section" id="acervo"><div className="catalog-heading"><div><p className="section-kicker">Acervo 2026</p><h2>Obras disponíveis</h2><p>{availableCount} de {works.length} obras desta prévia ainda disponíveis.</p></div><div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: "16px" }}><div className="filters" aria-label="Filtrar obras"><button className={filter === "available" ? "active" : ""} onClick={() => setFilter("available")}>Disponíveis</button><button className={filter === "sold" ? "active" : ""} onClick={() => setFilter("sold")}>Adquiridas</button><button className={filter === "all" ? "active" : ""} onClick={() => setFilter("all")}>Todas</button></div></div></div>
      <div style={{ position: "relative" }}>
        <button className="floating-arrow left" onClick={() => scrollCarousel('left')} aria-label="Rolar para a esquerda">←</button>
        <button className="floating-arrow right" onClick={() => scrollCarousel('right')} aria-label="Rolar para a direita">→</button>
        <div className="works-grid" aria-live="polite" ref={carouselRef}>{visibleWorks.map((work) => {
          const inCart = cart.find((item) => item.work.id === work.id);
          return <article className={`work-card ${work.status !== "available" ? "unavailable" : ""}`} key={work.id}><div className={`work-image ${work.palette}`}><span className={`status ${work.status}`}>{statusLabel[work.status]}</span><div className="art-shape shape-one" /><div className="art-shape shape-two" />{work.status !== "available" && <div className="sold-overlay"><strong>{statusLabel[work.status]}</strong><span>Esta obra não está disponível para proposta.</span></div>}</div><div className="work-body"><span className="work-code">{work.code}</span><h3>{work.title}</h3><p>{work.artist}</p><dl><div><dt>Técnica</dt><dd>{work.technique}</dd></div><div><dt>Dimensões</dt><dd>{work.dimensions}</dd></div><div><dt>Lance</dt><dd>{work.status !== "available" ? "Encerrado" : inCart ? money.format(inCart.amount / 100) : "Definido por você"}</dd></div></dl><button disabled={work.status !== "available"} onClick={() => addToCart(work)}>{inCart ? "Na sua sacola" : "Adicionar à sacola"}<span aria-hidden="true">→</span></button></div></article>;
        })}</div>
      </div>
    </section>
    <section className="how-section" id="como-comprar"><div><p className="section-kicker light">Como participar</p><h2>Envie sua proposta.<br />Nós cuidamos do restante.</h2></div><ol><li><span>01</span><div><strong>Selecione</strong><p>Escolha as obras de seu interesse e adicione à sacola.</p></div></li><li><span>02</span><div><strong>Faça sua proposta</strong><p>Defina o valor do seu lance para cada obra selecionada e envie.</p></div></li><li><span>03</span><div><strong>Aguarde o evento</strong><p>Após enviar sua proposta, aguarde o dia do evento para o anúncio dos lances vencedores.</p></div></li></ol></section>
