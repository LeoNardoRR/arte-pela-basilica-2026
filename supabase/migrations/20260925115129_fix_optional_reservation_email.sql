-- Keep the public reservation RPC aligned with the optional email field.
-- Preserve the production hold, special artwork, and internal notification behavior.
CREATE OR REPLACE FUNCTION public.submit_pre_reservation(bidder_name text, bidder_email text, bidder_phone text, items jsonb, extra_contribution_cents integer DEFAULT 0, hold_minutes integer DEFAULT 60)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
...
begin
  if char_length(clean_name) not between 2 and 120
    or (clean_email <> '' and (
      char_length(clean_email) not between 5 and 160
    ...
    raise exception 'Preencha nome e WhatsApp corretamente. Se informar e-mail, confira o endereço.' using errcode = '22023';
  end if;
  ...
  where c.created_at >= now() - interval '1 hour'
    and ((clean_email <> '' and lower(c.bidder_email) = clean_email)
...
-- An omitted buyer email must not create an undeliverable buyer message.
...
begin
  ...
  if btrim(coalesce(cart.bidder_email, '')) <> '' then
